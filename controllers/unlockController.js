import fs from "fs";
import path from "path";
import { decryptPDF } from "@pdfsmaller/pdf-decrypt";
import { v4 as uuidv4 } from "uuid";
import File from "../models/File.js";
import Job from "../models/Job.js";

export const unlockPdf = async (req, res) => {
  let job = null;
  const inputPath = req.file ? req.file.path : null;

  try {
    if (!req.file) {
      return res.status(400).json({ error: "No PDF file uploaded" });
    }

    // 1. Validate PDF signature
    const buffer = Buffer.alloc(5);
    const fd = fs.openSync(inputPath, "r");
    fs.readSync(fd, buffer, 0, 5, 0);
    fs.closeSync(fd);

    if (buffer.toString("ascii") !== "%PDF-") {
      fs.unlinkSync(inputPath);
      return res.status(400).json({ error: "Uploaded file is not a valid PDF" });
    }

    const { password } = req.body;

    if (!password || typeof password !== "string" || password.trim() === "") {
      fs.unlinkSync(inputPath);
      return res.status(400).json({ error: "password parameter is required and must be a non-empty string" });
    }

    // 2. Create Job & File records
    job = await Job.create({
      tool: "unlock",
      status: "processing"
    });

    await File.create({
      originalName: req.file.originalname,
      fileName: req.file.filename,
      path: req.file.path,
      mimeType: req.file.mimetype,
      size: req.file.size
    });

    // 3. Attempt to decrypt the PDF bytes
    const pdfBytes = fs.readFileSync(inputPath);
    let decryptedBytes;
    try {
      decryptedBytes = await decryptPDF(new Uint8Array(pdfBytes), password);
    } catch (decryptErr) {
      // Clean up uploaded encrypted file on decryption failure (e.g. wrong password)
      fs.unlinkSync(inputPath);
      
      job.status = "error";
      await job.save().catch(() => {});

      return res.status(400).json({
        error: "Incorrect password or decryption failed. Please verify the password."
      });
    }

    const outFileName = `unlock-conv-${Date.now()}-${uuidv4()}.pdf`;
    const outPath = path.resolve("uploads", "output", outFileName);

    // Ensure directory exists and write file
    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, decryptedBytes);

    const downloadUrl = `/uploads/output/${outFileName}`;

    // 4. Update Job status
    job.status = "done";
    job.outputFile = outFileName;
    await job.save();

    return res.status(200).json({
      jobId: job._id,
      status: "done",
      downloadUrl
    });

  } catch (error) {
    console.error("unlockPdf error:", error);

    if (inputPath && fs.existsSync(inputPath)) {
      try {
        fs.unlinkSync(inputPath);
      } catch (unlinkErr) {
        console.error("Could not unlink source PDF file on error:", unlinkErr);
      }
    }

    if (job) {
      job.status = "error";
      await job.save().catch(() => {});
    }

    return res.status(500).json({
      error: error.message || "Internal server error during PDF decryption"
    });
  }
};
